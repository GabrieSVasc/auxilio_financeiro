...
/**
 * Classe ligada ao fxml da tela de dados de desconto
 * 
 * @author Maria Gabriela
 */

public class DadosDescontoViewController implements Initializable{
	@FXML
	...
	protected void btnConfirmarAction(ActionEvent e) {
		...
		Parametros p = new Parametros(2, tipoDesconto, spinnerValor.getValue().doubleValue(), spinnerTaxa.getValue().doubleValue(), spinnerTempo.getValue().doubleValue(), tipoDuracao);
		try {
			RetornoInvestimento r = fachada.investimentos(p);
			Alert alerta = new Alert(AlertType.INFORMATION);
			alerta.setTitle(tipoDescontoStr);
