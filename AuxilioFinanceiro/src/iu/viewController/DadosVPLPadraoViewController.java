...
public class DadosVPLPadraoViewController implements Initializable {
	...
	protected void btnConfirmarAction(ActionEvent e) {
		...
		Parametros p = new Parametros(5, 1, spinnerCusto.getValue().doubleValue(), spinnerTaxa.getValue().doubleValue(), spinnerTempo.getValue().doubleValue(), tipoDuracao);
		p.setArrecadacao(txtArrecadacao.getText());
		try {
			RetornoInvestimento r = fachada.investimentos(p);
			Alert alerta = new Alert(AlertType.INFORMATION);
			alerta.setTitle("Valor Presente Líquido");
