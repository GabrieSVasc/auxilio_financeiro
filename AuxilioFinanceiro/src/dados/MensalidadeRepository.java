...
public class MensalidadeRepository {
	...
	/**
	 * Construtor que inicializa a lista de mensalidade
	 */
	...
	/**
	 * Método que retorna todas as mensalidades cadastradas
	 * 
	 * @return Lista com todas as mensalidades cadastradas
	 */
	...
	/**
	 * Método que adiciona uma mensalidade e salva ela no arquivo
	 * 
	...
	/**
	 * Método que remove uma mensalidade e salva o arquivo sem ela
	 * 
	 * @param m Mensalidade a ser removida
	 */
	...
	/**
	 * Método que atualiza uma mensalidade e salva o arquivo com ela
	 * 
	 * @param m                  Mensalidade a ser atualizada
	 * @param novoNome           Novo nome
	 * @param novoValor          Novo valor
	 * @param novaDataVencimento Nova data de vencimento
	 * @param novaRecorrencia    Nova recorrencia
	 * @param novoStatusPago     Novo status da mensalidade
	 * @throws CampoVazioException
	 */
	public void atualizar(Mensalidade m, String novoNome, double novoValor, LocalDate novaDataVencimento,
			String novaRecorrencia, boolean novoStatusPago) throws CampoVazioException {
		int indice = mensalidades.indexOf(m);
		if (indice != -1) {
	...
	/**
	 * Método que consulta uma mensalidade
	 * 
	 * @param id Id da mensalidade
	 * @return A mensalidade se encontrada ou null se não
	 */
	...
	/**
	 * Salva a lista de mensalidades no arquivo. Converte cada objeto Mensalidade
	 * para uma string formatada e delega a escrita para utilitário.
	 */
	private void salvar() {
	...
	/**
	 * Carrega a lista de mensalidades do arquivo, reconstruindo os objetos a partir
	 * das linhas lidas. Recebe o manager de categorias para associar corretamente
	 * cada mensalidade à sua categoria.
	 * 
		...
		for (String linha : linhas) {
			try {
				...
				if (categoria != null) {
					...
					mensalidades.add(new Mensalidade(id, nome, valor, dataCriacao, categoria, dataVencimento,
							recorrencia, isPago));
				} else {
					// Caso a categoria não seja encontrada, exibe aviso e ignora a mensalidade
					System.err.println("Aviso: Categoria com ID " + idCategoria + " não encontrada para a mensalidade '"
							+ nome + "'.");
				}
			} catch (Exception e) {
